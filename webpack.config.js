  ...
  apply(compiler) {
    compiler.hooks.done.tap("Update functions.php", function () {
      ...
      fse.readFile("./functions.php", "utf8", function (err, data) {
        ...
        let result = data.replace(scriptsRegEx, `${manifest["scripts.js"]}'`).replace(cssRegEx, `${manifest["scripts.css"]}'`)
...
if (currentTask == "build" || currentTask == "buildWatch") {
  ...
  config.output = {
    publicPath: "/wp-content/themes/grupa-kontratak-website/bundled-assets/",
